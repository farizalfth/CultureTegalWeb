...
from app.models import db, CultureSite, Event, UMKM, User
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password) and user.role == 'admin': # type: ignore
            # Aktifkan sesi permanen lintas browser close
...
def update_budaya(id):
    ...
    try:
        site.nama_tempat = request.form.get('nama_tempat') or site.nama_tempat # type: ignore
        site.subtitle = request.form.get('subtitle') # type: ignore
        site.kategori = request.form.get('kategori') or site.kategori # type: ignore
        site.tahun_dibangun = request.form.get('tahun_dibangun') # type: ignore
        site.durasi_kunjungan = request.form.get('durasi_kunjungan') # type: ignore
        site.jarak_estimasi = request.form.get('jarak_estimasi') # type: ignore
        site.deskripsi = request.form.get('deskripsi') or site.deskripsi # type: ignore
        site.fun_fact = request.form.get('fun_fact') # type: ignore
        ...
        site.video_url = request.form.get('video_url') # type: ignore
        site.is_slider = True if request.form.get('is_slider') == 'on' else False # type: ignore

        file_utama = request.files.get('gambar_utama')
        if file_utama and file_utama.filename != '':
            delete_file(site.image_url) # type: ignore
            site.image_url = save_image(file_utama) # type: ignore

        gallery_files = request.files.getlist('gallery_images')
        new_gallery_pics = [save_image(f) for f in gallery_files if f.filename != '']
        if new_gallery_pics:
            current_gallery = list(site.gallery) if site.gallery else [] # type: ignore
            site.gallery = current_gallery + new_gallery_pics # type: ignore
...
def delete_gallery_img(id, filename):
    ...
    if site.gallery and filename in site.gallery:
        ...
        site.gallery = updated_gallery # type: ignore
...
def update_event(id):
    ...
    try:
        if start_date:
            event.raw_date = datetime.strptime(start_date, '%Y-%m-%d').date() # type: ignore
            event.raw_end_date = datetime.strptime(end_date, '%Y-%m-%d').date() if end_date else event.raw_date # type: ignore
            event.tanggal_lengkap = format_indo_date(start_date, end_date) # type: ignore
        
        if start_t and end_t:
            event.waktu_pelaksanaan = f"{start_t} - {end_t} WIB" # type: ignore

        event.judul_event = request.form.get('judul_event') or event.judul_event # type: ignore
        event.kategori = request.form.get('kategori') or event.kategori # type: ignore
        event.status = request.form.get('status') or event.status # type: ignore
        event.lokasi_singkat = request.form.get('lokasi_singkat') or event.lokasi_singkat # type: ignore
        event.alamat_lengkap = request.form.get('alamat_lengkap') or event.alamat_lengkap # type: ignore
        event.audience = request.form.get('audience') or event.audience # type: ignore
        event.harga_tiket = request.form.get('harga_tiket') or event.harga_tiket # type: ignore
        event.deskripsi = request.form.get('deskripsi') or event.deskripsi # type: ignore
        ...
        event.badge_top = request.form.get('badge_top') or event.badge_top # type: ignore
        event.badge_bottom = request.form.get('badge_bottom') or event.badge_bottom # type: ignore
        event.is_recurring = True if request.form.get('is_recurring') == 'on' else False # type: ignore

        file = request.files.get('gambar_event')
        if file and file.filename != '':
            delete_file(event.image_url) # type: ignore
            event.image_url = save_image(file) # type: ignore
...
def update_umkm(id):
    ...
    try:
        umkm.nama_produk = request.form.get('nama_produk') or umkm.nama_produk # type: ignore
        umkm.harga = request.form.get('harga') or umkm.harga # type: ignore
        umkm.nama_toko = request.form.get('nama_toko') or umkm.nama_toko # type: ignore
        umkm.kategori = request.form.get('kategori') or umkm.kategori # type: ignore
        umkm.rating = float(request.form.get('rating') or umkm.rating) # type: ignore
        umkm.deskripsi = request.form.get('deskripsi') or umkm.deskripsi # type: ignore
        umkm.alamat_toko = request.form.get('alamat_toko') or umkm.alamat_toko # type: ignore
        umkm.no_whatsapp = request.form.get('no_whatsapp') or umkm.no_whatsapp # type: ignore
        umkm.link_eksternal = request.form.get('link_eksternal') or umkm.link_eksternal # type: ignore

        file = request.files.get('gambar_umkm')
        if file and file.filename != '':
            delete_file(umkm.image_url) # type: ignore
            umkm.image_url = save_image(file) # type: ignore
...
def update_user(id):
    ...
    try:
        user.nama = request.form.get('nama') or user.nama # type: ignore
        user.email = request.form.get('email') or user.email # type: ignore
        ...
        if user.role == 'admin':
            ...
            if new_pass:
                user.password_hash = generate_password_hash(new_pass) # type: ignore

        file = request.files.get('profile_picture')
        if file and file.filename != '':
            delete_file(user.profile_picture) # type: ignore
            user.profile_picture = save_image(file) # type: ignore
...
def delete_user(id):
    ...
    user = User.query.get_or_404(id)
    try:
        supabase.auth.admin.delete_user(str(id)) # type: ignore
        delete_file(user.profile_picture) # type: ignore
